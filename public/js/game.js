...
let leaderboard = [];
let bubbles = []; // Array to store bubble positions
...
const numShieldParticles = 20; // Number of particles
const particleSpeed = 0.05; // Speed of orbiting particles
...
function draw() {
    ...
    if (bubbles.length > 100) bubbles.shift(); // Limit the number of bubbles
    ...
    for (let i = 0; i < bubbles.length; i++) {
        ...
        context.beginPath();
        context.arc(bubble.x + cameraOffsetX, bubble.y + cameraOffsetY, bubble.radius, 0, Math.PI * 2);
        context.fill();
    }
    ...
    for (let id in players) {
        const p = players[id];
        if (p.invisible > serverTime && id !== socket.id) continue; // Skip drawing invisible players
        ...
        if (p.shield > serverTime) {
            ...
            shieldParticles.length = 0; // Clear particles when shield is not active
        }
